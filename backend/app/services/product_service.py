...
def _payload_dict(payload):
    ...
    # price/discount_price are Decimal on the Pydantic schema so ProductCreate
    # can validate "discount_price <= price" precisely. pymongo's BSON encoder
    # cannot serialize a plain Python Decimal (it raises
    # bson.errors.InvalidDocument: cannot encode object ... Decimal), so every
    # product create/update with a price set was failing at db.products.
    # insert_one()/update_one() with a 500 error before the document ever
    # reached MongoDB. Cast to float, Mongo's native numeric type, before
    # writing; ProductOut/Product schemas already accept float and coerce it
    # back to Decimal on the way out.
    for money_field in ("price","discount_price"):
