import { authService, customerService, productService } from './services.js';
import { COMPANY, CATEGORIES, SITE_PAGES } from './data.js';
  ...
  <article class="group bg-white rounded-2xl border border-fc-line shadow-card hover:shadow-soft transition-shadow duration-300 overflow-hidden flex flex-col">
    ...
    <div class="p-4 flex flex-col gap-2 flex-1">
      ...
      <div class="flex gap-2 pt-1">
        ...
        <button data-cart-add="${p.id}" ${p.stockStatus === 'Out of Stock' ? 'disabled' : ''} class="flex-1 text-sm font-medium border border-fc-green text-fc-green rounded-xl py-2 hover:bg-fc-greenlight transition-colors disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-transparent">Add to Cart</button><button data-getcode="${p.id}" ${p.stockStatus === 'Out of Stock' ? 'disabled' : ''} class="flex-1 text-sm font-medium bg-fc-green text-white rounded-xl py-2 hover:bg-fc-greendark transition-colors disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-fc-green">Get a Quote</button>
      </div>
...
export function header() {
  ...
  <span class="font-display font-bold text-base sm:text-lg tracking-tight leading-none whitespace-nowrap">
    FARM CRAFT
        ...
        <form id="header-search" class="hidden md:flex relative items-center flex-1 max-w-xs bg-fc-offwhite border border-fc-line rounded-full px-3 py-2 focus-within:border-fc-green transition-colors">
          <i data-lucide="search" class="w-4 h-4 text-fc-slate/60 shrink-0"></i>
          <input name="q" type="search" autocomplete="off" placeholder="Search grain machinery…" class="bg-transparent outline-none text-sm px-2 w-full placeholder:text-fc-slate/50" />
          <div data-search-dropdown class="hidden absolute top-full left-0 right-0 mt-2 bg-white border border-fc-line rounded-xl shadow-soft max-h-80 overflow-y-auto thin-scroll z-50 text-left"></div>
        </form>

        <div class="flex items-center gap-1 sm:gap-2">
          <a href="#/cart" class="relative w-10 h-10 rounded-full hover:bg-fc-greenlight flex items-center justify-center transition-colors" aria-label="Cart">
            <i data-lucide="shopping-cart" class="w-5 h-5"></i>
            <span id="cart-count-badge" class="absolute -top-0.5 -right-0.5 bg-fc-wheat text-white text-[10px] font-bold rounded-full w-4 h-4 flex items-center justify-center hidden">0</span>
          </a>
      ...
      <form id="header-search-mobile" class="md:hidden relative pb-3 flex items-center bg-fc-offwhite border border-fc-line rounded-full px-3 py-2">
        <i data-lucide="search" class="w-4 h-4 text-fc-slate/60 shrink-0"></i>
        <input name="q" type="search" autocomplete="off" placeholder="Search grain machinery…" class="bg-transparent outline-none text-sm px-2 w-full placeholder:text-fc-slate/50" />
        <div data-search-dropdown class="hidden absolute top-full left-0 right-0 mt-2 bg-white border border-fc-line rounded-xl shadow-soft max-h-80 overflow-y-auto thin-scroll z-50 text-left"></div>
      </form>
...
// Closes any open search-results dropdown when the customer clicks outside
// it. Bound to `document` exactly once for the page's lifetime (guarded by
// the module-level flag below) rather than inside attachHeaderBehaviour,
// which runs on every render — the header markup itself is replaced each
// render so its own listeners are naturally cleaned up, but a listener
// added to `document` is not, and would otherwise pile up on every
// navigation.
let searchOutsideClickBound = false;
function ensureSearchOutsideClickHandler() {
  if (searchOutsideClickBound) return;
  searchOutsideClickBound = true;
  document.addEventListener('click', (e) => {
    document.querySelectorAll('[data-search-dropdown]:not(.hidden)').forEach(dd => {
...
function searchResultRow(p) {
  return `
    <a href="#/product/${p.slug}" class="flex items-center gap-3 px-3 py-2.5 hover:bg-fc-offwhite transition-colors border-b border-fc-line/60 last:border-0">
      ${imgWithFallback(p.images[0], '', 'w-10 h-10 rounded-lg object-cover bg-fc-greenlight shrink-0')}
      <div class="min-w-0 flex-1">
        <div class="text-sm font-medium truncate">${escapeHtml(p.name)}</div>
        <div class="text-xs text-fc-slate/55 truncate">${escapeHtml((p.category || '').replace(/-/g, ' '))}</div>
      </div>
      ${stockBadge(p.stockStatus)}
    </a>`;
}

// Matches customer-site pages/modules (Home, Products, About Us, Services,
// Contact, Wishlist, Cart, My Profile, My Orders) against a search term —
// by page name or one of its keyword aliases — so search finds pages, not
// only products.
...
function pageResultRow(page) {
  return `
    <a href="${page.href}" class="flex items-center gap-3 px-3 py-2.5 hover:bg-fc-offwhite transition-colors border-b border-fc-line/60 last:border-0">
      <span class="w-10 h-10 rounded-lg bg-fc-greenlight flex items-center justify-center shrink-0">
        <i data-lucide="${page.icon}" class="w-4 h-4 text-fc-green"></i>
      </span>
      <div class="min-w-0 flex-1">
        <div class="text-sm font-medium truncate">${escapeHtml(page.name)}</div>
        <div class="text-xs text-fc-slate/55 truncate">Page</div>
      </div>
    </a>`;
}
...
export function attachHeaderBehaviour(router) {
  ...
  ensureSearchOutsideClickHandler();
  ...
  ['header-search', 'header-search-mobile'].forEach(id => {
    ...
    // Renders live, as-you-type matches: customer-site pages/modules first
    // (Home, Products, About Us, Services, Contact, Wishlist, Cart, My
    // Profile, My Orders), then products. Debounced (300ms) below so this
    // runs once after the customer pauses typing, not on every keystroke.
    //
    // Product matches come from the in-memory catalog, so this never fires
    // a network request itself — but search must not simply fail because
    // the catalog hasn't finished its background load yet (main.js kicks
    // that off on boot). While it's still loading, this shows a "Searching…"
    // state and kicks off/awaits the load, then re-renders once it lands —
    // as long as the customer hasn't changed the query in the meantime.
    function renderDropdown(rawQuery) {
      ...
      const pageMatches = matchPages(q).slice(0, 4);

      if (productService.loading || (!productService.loaded && !productService.error)) {
        dropdown.innerHTML = pageMatches.map(pageResultRow).join('') + `
          <div class="px-4 py-4 text-center text-sm text-fc-slate/60">Searching products…</div>`;
        dropdown.classList.remove('hidden');
        icons();

        productService.load().then(() => {
          // Only re-render if the customer is still searching the same term.
          if (input.value.trim() === q) renderDropdown(q);
        });
        return;
      }

      const productMatches = productService.search(q).slice(0, 6);
      const matches = [...pageMatches, ...productMatches];

      if (!matches.length) {
        const note = productService.error
          ? `Products are unavailable right now — ${escapeHtml(productService.error)}`
          : `No results found for &ldquo;${escapeHtml(q)}&rdquo;`;
        dropdown.innerHTML = `<div class="px-4 py-6 text-center text-sm text-fc-slate/60">${note}</div>`;
        dropdown.classList.remove('hidden');
        return;
      }

      dropdown.innerHTML =
        pageMatches.map(pageResultRow).join('') +
        productMatches.map(searchResultRow).join('') + `
        <button type="submit" class="w-full text-center text-sm font-medium text-fc-green px-3 py-2.5 hover:bg-fc-greenlight/50 transition-colors">
          See all results for &ldquo;${escapeHtml(q)}&rdquo;
        </button>`;
      dropdown.classList.remove('hidden');
      icons();
    }
    ...
    input?.addEventListener('input', () => {
      ...
      debounceTimer = setTimeout(() => renderDropdown(value), 300);
    });
    ...
    form.addEventListener('submit', (e) => {
      ...
      // If the submitted term is an exact/near-exact match for a single
      // page/module (e.g. "cart", "my orders", "wishlist"), open that page
      // directly instead of running it as a product search.
      const exactPage = SITE_PAGES.find((page) =>
        page.name.toLowerCase() === ql ||
        (page.keywords || []).includes(ql)
      );

      window.location.hash = exactPage
        ? exactPage.href
