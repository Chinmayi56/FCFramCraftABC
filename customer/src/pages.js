    ...
    <div class="grid lg:grid-cols-2 gap-10 mb-16">
      ...
      <div>
        ...
        <div class="grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)_3rem] sm:flex gap-2.5 sm:gap-3 mb-3 items-stretch">
          ...
          <button data-getcode="${p.id}" ${p.stockStatus === 'Out of Stock' ? 'disabled' : ''} class="min-w-0 sm:flex-1 bg-fc-green text-white font-medium py-3 rounded-xl hover:bg-fc-greendark transition-colors disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-fc-green">Get a Quote</button>
...
export function attachProductDetailPage() {
  ...
  // Out-of-stock / low-stock notice, driven by real backend stock status.
  // Shown once per product+status per browser session (sessionStorage-keyed)
  // so it never repeats on every render/navigation back to the same page —
  // and it's never shown for products that are actually in stock.
  const slug = window.location.hash.replace(/^#/, '').split('?')[0].replace('/product/', '');
  const p = productService.bySlug(slug);

  if (p && (p.stockStatus === 'Out of Stock' || p.stockStatus === 'Low Stock')) {
    const seenKey = `fc_stock_notice::${p.id}::${p.stockStatus}`;

    if (!sessionStorage.getItem(seenKey)) {
      sessionStorage.setItem(seenKey, '1');

      const message = p.stockStatus === 'Out of Stock'
        ? `${p.name} is currently out of stock. Please contact us for availability.`
        : `${p.name} is booking fast — limited stock remaining.`;

      toast(message, { type: p.stockStatus === 'Out of Stock' ? 'error' : undefined });
    }
  }
}
...
export function attachCartPage(router){
  // Re-render in place via `router` (not window.location.hash = '#/cart')
  // because the hash is already '#/cart' here — reassigning the same hash
  // does not fire a 'hashchange' event, so the cart list, totals and the
  // header cart badge would silently go stale after every quantity change
  // or removal until the customer navigated elsewhere and back.
  document.querySelectorAll('[data-cart-remove]').forEach(b=>b.addEventListener('click',async()=>{await cartService.remove(b.dataset.cartRemove); router?.({resetScroll:false});}));
  document.querySelectorAll('[data-cart-inc],[data-cart-dec]').forEach(b=>b.addEventListener('click',async()=>{const item=(await cartService.get()).items.find(x=>x.id===b.dataset.cartInc||x.id===b.dataset.cartDec); if(!item)return; const q=Math.max(1,item.quantity+(b.dataset.cartInc?1:-1)); await cartService.update(item.id,q); router?.({resetScroll:false});}));
