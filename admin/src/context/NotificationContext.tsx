...
export function NotificationProvider({ children }: { children: ReactNode }) {
  ...
  //
  // Two things previously caused the same real order to produce more than
  // one notification:
  //  1. The "seen" ids were only written to localStorage *after* every
  //     order in the poll had already been turned into a notification. If
  //     a poll's network request took longer than the poll interval (or, in
  //     development, React 18 StrictMode's intentional double-invoke of
  //     effects), a second poll could start before the first had persisted
  //     its results — both would see the same order as "unseen" and both
  //     would call addNotification for it.
  //  2. Nothing prevented two poll cycles from ever running concurrently.
  //
  // The fix: guard against overlapping polls with a ref-based lock, and
  // mark each order as seen (in-memory, synchronously) the instant we
  // decide to notify for it — before the async addNotification/localStorage
  // write even starts — so a concurrent poll can never observe it as
  // unseen. A `cancelled` flag also protects against a stale effect (e.g.
  // from StrictMode's mount → unmount → remount cycle) writing after its
  // own cleanup has already run.
  useEffect(() => {
    let cancelled = false;
    ...
    const baseline = async () => {
      ...
      try {
        ...
        // Ignore — the next poll cycle will retry.
      }
    };

    const poll = async () => {
      if (isPollingRef.current) return; // a previous poll is still in flight
      ...
      try {
        ...
        if (unseen.length > 0) {
          // Mark seen immediately (synchronously, before any notification is
          // created) so an overlapping poll can never notify for the same order.
          markOrderIdsSeen(orders.map((o) => o.id));

          // Deduplicate within this batch too, in case the backend ever
          // returns the same order id more than once in one response.
          const notifiedIds = new Set<string>();
          unseen.forEach((order) => {
            if (notifiedIds.has(order.id)) return;
    ...
    baseline();
    const interval = window.setInterval(() => { poll(); }, NEW_ORDER_POLL_MS);
    return () => {
      cancelled = true;
