...
export default function NotificationBell() {
  ...
  // Clicking a notification always marks it read. For an order notification
  // with a known order id, it also closes the dropdown and takes the admin
  // straight to that order's existing Order Detail page — the same route
  // the Purchased Products list's View/Eye button opens.
  const handleOpen = (n: AdminNotification) => {
    markRead(n.id);
    if (n.type === "order" && n.orderId) {
      setOpen(false);
      navigate(`/admin/purchased-products/${encodeURIComponent(n.orderId)}`);
    }
  };
