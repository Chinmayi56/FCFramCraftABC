...
export interface AdminNotification {
  ...
  read: boolean;
  // Stable identifier for the real-world event this notification represents
  // (e.g. an order id + event type). Used to reject duplicate inserts of the
  // same event even if `addNotification` is ever called for it twice.
  dedupeKey?: string;
  // The order this notification is about, when type === "order" — lets the
  // bell navigate straight to that order's Order Detail page on click.
  orderId?: string;
}
...
export function addNotification(input: {
  ...
  title: string;
  message: string;
  // e.g. `order:${orderId}:new` — pass this for any event that must never
  // be shown twice (a real order should always produce exactly one
  // notification, however many times the event source fires).
  dedupeKey?: string;
  orderId?: string;
}): AdminNotification[] {
  const list = loadNotifications();

  if (input.dedupeKey && list.some((n) => n.dedupeKey === input.dedupeKey)) {
    // Same event already recorded — never insert a second notification for it.
    return list;
  }
